...
2. Perform sample-level QC.
3. Perform variant-level QC using a allele-specific VQSR model.


## Usage

The workflow is configured via a CPG_CONFIG_PATH TOML file, e.g. with `prophecy-test.toml`:

```sh
export CPG_CONFIG_PATH=large_cohort/configs/prophecy-test.toml
