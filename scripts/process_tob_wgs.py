...
def filter_files(files: List[str], prefix: str):
    ...
    for file_path in files:
        ...
        if file_path.endswith(('.g.vcf.gz', '.g.vcf.gz.tbi', '.g.vcf.gz.md5', 'csv')):
...
if __name__ == '__main__':
    ...
    file_list = open('../test/data/tob_wgs_batch2.txt')
    lines = file_list.readlines()
    PREF = 'gs://cpg-tob-wgs-upload/'
    main_files, archive_files = filter_files(lines, PREF)
    ...
    main_batch.run()
    archive_prefix = os.path.join('vivian-dev-archive', 'cram', 'batch2')
