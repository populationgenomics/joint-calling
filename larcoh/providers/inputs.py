...
class InputProvider(ABC):
        ...
        if dataset_names is not None:
            ...
            for ds_name in dataset_names:
                if skip_datasets and ds_name in skip_datasets:
                    ...
                    continue
                dataset = Dataset.create(ds_name)
    ...
    def filter_sequencing_type(cohort: Cohort, sequencing_type: SequencingType | None):
        ...
        for s in cohort.get_samples():
            if not s.sequencing_meta_by_type:
                logger.warning(f'{s}: skipping because no sequencing found')
                s.active = False
                continue

            avail_types = list(s.sequencing_meta_by_type.keys())
