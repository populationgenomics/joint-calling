...
@click.option(
    '--model-path',
    'model_path',
    help='Model file created by --train_rf and only needed for --apply_rf without '
    'running --train_rf.',
)
@click.option(
    '--model-id',
    'model_id',
    help='Model ID generated by --train_rf, and only needed for --apply_rf without '
    'running --train_rf.',
)
# @click.option(
#     '--annotate-for-rf',
...
@click.option(
    '--vqsr-training',
    'vqsr_training',
    help='Use VQSR training examples.',
    is_flag=True,
)
# @click.option(
#     '--vqsr-model-id',
...
def main(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    #     work_bucket,
    #     f'rf/rf_annotations_{'adj' if adj else 'raw'}.ht'
    # )
    # rf_model_path = join(work_bucket, 'rf/models/{model_id}/rf.model')
    # rf_training_ht_path = join(work_bucket, 'rf/models/{model_id}/training.ht')
    ...
    if model_path:
        ...
        assert training_ht_path and file_exists(training_ht_path)
        model = load_model(model_path)
        training_ht = hl.read_table(training_ht_path)
    else:
